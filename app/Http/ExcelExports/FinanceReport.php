...
class FinanceReport implements FromView, WithEvents
{
    ...
    {
        return view('report.finance_report_xlsx', [
    ...
    public static function afterSheet(AfterSheet $event)
    {
        ...
        $event->sheet->getColumnDimension('H')->setWidth(21);
        $event->sheet->getStyle('A1:H2')->applyFromArray($headingArray);
        $event->sheet->getStyle('A4:H4')->applyFromArray($styleArray);
        $count = $event->sheet->getHighestDataRow();
        $event->sheet->getStyle("A5:H{$count}")->applyFromArray($normalStyle);
        for ($x = 5; $x <= $count; $x++) {
